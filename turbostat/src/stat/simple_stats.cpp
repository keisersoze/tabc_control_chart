...
double wilcoxon_rank_sum (const std::vector<double> &x1,
    ...
    double obs_stat = std::accumulate(ranks.begin(), ranks.begin() + x1.size(), 0.0);
    return obs_stat;
}
...
double centered_wilcoxon_rank_sum (const std::vector<double> &x1,
    ...
    double N = m + (double) x2.size();
    double expected_value = m * (N + 1.0) / 2.0;
    return wilcoxon_rank_sum(x1,x2) - expected_value;
}
